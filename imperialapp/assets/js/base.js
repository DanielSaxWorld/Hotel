//-----------------------------------------------------------------------
// Version:        2.1
// Template name:  FirstOctet - Wallet & Banking HTML Mobile Template
// Item URL :      https://themeforest.net/item/FirstOctet-wallet-banking-html-mobile-template/25738217
// Author:         Bragher
...
goBackButton.forEach(function(el) {
        el.addEventListener("click", function() {
            if (FirstOctet.Animation.goBack) {
                goBackAnimation();
            } else {
                window.history.go(-1);
            }
...
function AddtoHome(time, once) {
    if (once) {
        var AddHomeStatus = localStorage.getItem("FirstOctetAddtoHome");
        if (AddHomeStatus === "1" || AddHomeStatus === 1) {
            // already showed up
        } else {
            localStorage.setItem("FirstOctetAddtoHome", 1)
...
//-----------------------------------------------------------------------
// Dark Mode
var checkDarkModeStatus = localStorage.getItem("FirstOctetDarkmode");
var switchDarkMode = document.querySelectorAll(".dark-mode-switch");
var pageBodyActive = pageBody.classList.contains("dark-mode");

// Check if enable as default
if (FirstOctet.Dark_Mode.default) {
    pageBody.classList.add("dark-mode");
}

// Local Dark Mode
if (FirstOctet.Dark_Mode.local_mode.enable) {
    var nightStart = FirstOctet.Dark_Mode.local_mode.start_time;
    var nightEnd = FirstOctet.Dark_Mode.local_mode.end_time;
...
if (FirstOctet.Dark_Mode.auto_detect.enable)
...
switchDarkMode.forEach(function(el) {
        el.addEventListener("click", function() {
            var darkmodeCheck = localStorage.getItem("FirstOctetDarkmode");
            var bodyCheck = pageBody.classList.contains('dark-mode');
            if (darkmodeCheck === 1 || darkmodeCheck === "1" || bodyCheck) {
                pageBody.classList.remove("dark-mode");
                localStorage.setItem("FirstOctetDarkmode", "0");
                switchDarkModeCheck(false);
            } else {
                pageBody.classList.add("dark-mode")
                switchDarkModeCheck(true);
                localStorage.setItem("FirstOctetDarkmode", "1");
            }
        })
    })
    //-----------------------------------------------------------------------
...
function testMode() {
    ...
    console.clear();
    console.log("%cFirstOctet", "font-size: 1.3em; font-weight: bold; color: #FFF; background-color: #13B5E6; padding: 10px 120px; margin-bottom: 16px;")
    ...
    function testModeInfo(value, msg) {
        console.log("%c|" + "%c " + msg + " : " + "%c" + value, "color: #444; font-size :1.2em; font-weight: bold;", "color: inherit", "color:#13B5E6; font-weight: bold;");
    }
    ...
    testModeMsg(FirstOctet.PWA.enable, "PWA")
    testModeMsg(FirstOctet.Dark_Mode.default, "Set dark mode as default theme")
    testModeMsg(FirstOctet.Dark_Mode.local_mode.enable, "Local dark mode (between " + FirstOctet.Dark_Mode.local_mode.start_time + ":00 and " + FirstOctet.Dark_Mode.local_mode.end_time + ":00)")
    testModeMsg(FirstOctet.Dark_Mode.auto_detect.enable, "Auto detect dark mode")
    testModeMsg(FirstOctet.RTL.enable, "RTL")
    testModeMsg(FirstOctet.Test.enable, "Test mode")
    testModeMsg(FirstOctet.Test.alert, "Test mode alert")
    ...
    testModeMsg(FirstOctet.Animation.goBack, "Go Back")
}

function themeTesting() {
    var word = FirstOctet.Test.word;
    var value = "";
