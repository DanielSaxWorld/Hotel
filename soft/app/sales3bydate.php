...
$sql = "SELECT *, DATE_FORMAT(date, '%a-%d-%b-%Y') as newsalesdate FROM `dsales_order` WHERE `date` BETWEEN '$datefrom' AND '$dateto' ORDER by 'transactions_id'";
...
?>
                    ...
                    <tbody>
<?php
 $sql = "SELECT *, sum(amount) as TOTAL_amount, sum(qty) as TOTAL_qty FROM dsales_order WHERE `date` BETWEEN '$datefrom' AND '$dateto'";
$result = mysqli_query($conn, $sql);
while($row = mysqli_fetch_array($result)) {
