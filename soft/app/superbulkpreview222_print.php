...
<body class="white-bg" style="font-size:11px; padding:0px">
                            ...
                            <h6 align="center"><strong>BULK SALES RECEIPT</strong></h6>

                            <h6 style="padding-left:0px"><strong>CUSTOMER NAME: <?php echo $cname; ?></strong></h6>
				...
				?>
						...
						<?php
					$id=$_GET['invoice'];
					$result = $db->prepare("SELECT *, SUM(qty) AS TotalItem FROM sales_order WHERE invoice= :userid");
					$result->bindParam(':userid', $id);
					$result->execute();
					for($i=0; $row = $result->fetch(); $i++){
						$_REQUEST['TotalItem']= $row['TotalItem'];
				...
				?>
					...
					?>
						...
						<tr align="center">
                                    <td colspan="4" align="left">WE ACCEPT REFUNDS ON DRUGS ONLY IF RETURNED IN A RESALEABLE CONDITION AND PRODUCTION OF ORIGINAL RECEIPT OF PURCHASE. ALL OTHER GOODS ARE SOLD "AS IS" AND THERE ARE NO REFUNDS.<br>
                                    </td>
                                    </tr>
