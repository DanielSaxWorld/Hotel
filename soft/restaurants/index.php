<?php
	...
	$query = "SELECT * FROM `staff_id` WHERE phone='$phone' AND password=PASSWORD('$password') AND designation='Restaurants' AND `status`='Active'";
	$result = mysqli_query($conn,$query) or die(mysql_error());
		$rows = mysqli_num_rows($result);
        if($rows==1){
			$_SESSION['phone'] = $phone;
			echo '<script type="text/javascript"> window.open("../app/restaurants.php","_self");</script>'; // Redirect user to index.php
            }{
...
<html lang="en">
<head>
	<title>Restaurants Login | Imperial Boni Hotels & Resorts</title>
	<meta charset="UTF-8">
